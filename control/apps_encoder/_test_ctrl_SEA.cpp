...
test_ctrl_SEA(void* dt_ns_ref) {
	...
	controller_renishaw encoder("can1"); 
	...
	const double THETA_RAD_MAX = 1.5; 
	...
	// Link-side angle trajectory
	double q_ref = 0, q_ref_prev = 0; 
	double dt_q_ref = 0, dt_q_ref_prev = 0;  

    //Renishaw encoder
	double theta_sea_in    = 0; 
	double theta_sea_out   = 0; 

	double theta_sea       = 0, theta_sea_prev = 0;
	double dt_theta_sea = 0, dt_theta_sea_prev = 0;
	...
	double AMPL_Link_POS_RAD_REF =  0; 
	float  ampl_theta_rad_ref_fl    = -1.0; 
	float  ampl_torque_Nm_ref_fl    = -1.0; 
	float  ampl_link_pos_rad_ref_fl = -1; 
	...
	AMPL_TORQUE_Nm_REF = (double)ampl_torque_Nm_ref_fl;
		...
		while (ampl_link_pos_rad_ref_fl < 0) {
		printf("\n");
		printf("Enter link-side angle amplitude (rad): "); 
		scanf("%f", &ampl_link_pos_rad_ref_fl);  
	}
	AMPL_Link_POS_RAD_REF = (double)ampl_link_pos_rad_ref_fl;
	...
	printf("AMPL_Link_POS_RAD_REF = [%3.3lf]\n", AMPL_Link_POS_RAD_REF); 
	...
	/////////////////////////////////////////////////////////////////////////// 
	// Cascaded PI torque control: inner-loop velocity and outer-loop torque/theta_sea 
	/////////////////////////////////////////////////////////////////////////// 
    const int ORD_Cas_PI = 2; 
    
	// Inner-loop velocity control PI transfer function coefficients (continuous time):
    double K_pi_inner = 5; // k = 2, z = 20 work fine 
	double p_pi_inner = 0; 
	double z_pi_inner = 30;  
	...
	// Outer-loop position control (theta_sea loop) PI transfer function coefficients (continuous time):
    double K_pi_outer = 10; // 
	double p_pi_outer = 0;
	double z_pi_outer = 100;
	...
	double tau_cas_pi_theta_arr[ORD_Cas_PI + 1]   = {0, 0, 0};
  ...
  // Auxiliary variables:
	...
	///////////////////////////////////////////////////////////////////////////
	// Nonlinear disturbance observer-based torque controller (NDOBC)
	///////////////////////////////////////////////////////////////////////////
    ...
    double err_q = 0; 
  	...
  	///////////////////////////////////////////////////////////////////////////
	// Cascade PID position controller
	///////////////////////////////////////////////////////////////////////////   
    const int ORD_Cas_PI_POS = 1;
    
	// Inner-loop PI control PI transfer function coefficients (continuous time):
    double K_pi_inner_POS = 0.6; // 10 is suitable for position inner loop
	double p_pi_inner_POS = 0;
	double z_pi_inner_POS = 10; // 10 is suitable for position inner loop

	// Outer-loop link position control, PI transfer function coefficients (continuous time):
    double K_pi_outer_POS = 8; 
	double p_pi_outer_POS = 0; 
	double z_pi_outer_POS = 30; 

    // Cascaded PI position transfer function coefficients (discrete time):
	double B_cas_inner_POS[ORD_Cas_PI_POS + 1]  = {0, 0};
	double A_cas_inner_POS[ORD_Cas_PI_POS + 1]  = {0, 0};
	double B_cas_outer_POS[ORD_Cas_PI_POS + 1]  = {0, 0}; 
	double A_cas_outer_POS[ORD_Cas_PI_POS + 1]  = {0, 0}; 

	coeffs_tf_discr_pole_zero_tustin(B_cas_inner_POS, A_cas_inner_POS, K_pi_inner_POS, p_pi_inner_POS, z_pi_inner_POS, dt_s);
	coeffs_tf_discr_pole_zero_tustin(B_cas_outer_POS, A_cas_outer_POS, K_pi_outer_POS, p_pi_outer_POS, z_pi_outer_POS, dt_s);

	// Commanded cascaded PI torque:
	double Desire_cas_pi_outer_POS = 0;
	double tau_cas_pi_in_POS       = 0;

	// Auxiliary variables
	double error_cas_inner_POS          = 0;
	double error_cas_inner_POS_prev     = 0;
	double error_cas_outer_POS_prev     = 0;
	double Desire_cas_pi_outer_POS_prev = 0;
	double tau_cas_pi_in_POS_prev       = 0;
	double tau_cas_pi_in_damping_POS    = 0;

    ///////////////////////////////////////////////////////////////////////////
	// Direct PID torque control 
	///////////////////////////////////////////////////////////////////////////
    const int ORD_dir_PID_POS = 2;
	
	// PID parameters
	double K_dir_P_POS = 10;
	double K_dir_D_POS = 0.1;
	double K_dir_I_POS = 1;

    // PID tansfer function with low-pass filter (continuous time)
	double F_LP_POS_HZ   = 100; // low-pass cutoff frequency
    double w_lp_POS_rad  = 2*PI*F_LP_POS_HZ;  

	double K_dir_PID_POS    = K_dir_D_POS*w_lp_POS_rad;
	double b_dir_PID_POS_1	= K_dir_P_POS / K_dir_D_POS;
	double b_dir_PID_POS_2	= K_dir_I_POS / K_dir_D_POS;

	double a_dir_PID_POS_1	= w_lp_POS_rad;
	double a_dir_PID_POS_2	= 0;
 
	// DZ transfer function coefficients (discrete time):
	double B_dir_PID_POS[ORD_dir_PID_POS + 1] = {0, 0, 0};
	double A_dir_PID_POS[ORD_dir_PID_POS + 1] = {0, 0, 0};

	coeffs_tf_discr_numord2_denord2_tustin(B_dir_PID_POS, A_dir_PID_POS, K_dir_PID_POS, b_dir_PID_POS_1, b_dir_PID_POS_2, a_dir_PID_POS_1, a_dir_PID_POS_2, dt_s); 

	// Commanded direct PID torque:
	double tau_dir_PID_POS = 0; 
	double tau_dir_PID_SingPertur_POS = 0;
    
	// sample array
	double error_q_dir_PID_POS_arr[ORD_dir_PID + 1]     = {0, 0, 0}; 
	double tau_dir_PID_POS_arr[ORD_dir_PID + 1]         = {0, 0, 0}; 
  ...
  // Direct PID fraction:
	...
	if (USE_COMP_DIST) {
		...
		while (frac_fric_fl < 0) {
			printf("Enter Friction fraction (0..1): "); 
	...
	// Position control
	////////////////////////////////////////////////////
	// Cascade PI controller fraction:
	double frac_pos_Cascade_PI; 
	float  frac_pos_Cascade_PI_fl = -1.0;

	while (frac_pos_Cascade_PI_fl < 0) {
		printf("Enter Cascade PI Position control fraction (0..1): "); 
		scanf("%f", &frac_pos_Cascade_PI_fl);  
	}
	frac_pos_Cascade_PI = (double)frac_pos_Cascade_PI_fl;

	printf("\n");
	printf("frac_Cascade_PI_POS = [%3.3lf]\n",  frac_pos_Cascade_PI);
	printf("\n");
    // Cascade PI controller fraction: 
	double frac_pos_Direct_PID; 
	float  frac_pos_Direct_PID_fl = -1.0;

	while (frac_pos_Direct_PID_fl < 0) {
		printf("Enter Direct PID Position control fraction (0..1): "); 
		scanf("%f", &frac_pos_Direct_PID_fl);  
	}
	frac_pos_Direct_PID = (double)frac_pos_Direct_PID_fl;

	printf("\n");
	printf("frac_Direct_PID_POS = [%3.3lf]\n",  frac_pos_Direct_PID); 
	printf("\n");
	...
	encoder.read_ang_encoder(encoder_arr); 
  	double theta_sea_in_o = (double) encoder_arr[1]*PI/180.0; 
  	double theta_sea_out_o = (double) encoder_arr[0]*PI/180.0; 
    ...
    while (run_on) {
        ...
        torque_sea      =  tau_est_SEA_model(PRE_EXTENSION_LEGENTH, OFFSET_ANGLE, theta_sea);  // little different from my result above
        // torque velocity 
	   ...
	   // Torque of the SEA
		...
		// Link side position
			...
			q_ref = AMPL_Link_POS_RAD_REF*(1.0 - exp( -N_T*(t_s - t_step_s)/T_EXP_R) ) * sin(omega_ref*(t_s - t_step_s));
		}
		else
			q_ref  = 0;
		// link-side angle velocity 
		// tf_discr_io_ord1(&dt_theta_ref, B_TDEMFC_diff, A_TDEMFC_diff, theta_ref, &theta_ref_prev, &dt_theta_ref_prev); 
        ...
        if ( dt_theta_sea_m > 0 ) // || (dt_theta_sea_m = 0 && tau_cmd_in > 0))
		{
		    sigma_adap_fric =  1;
			}
		 //else if (dt_theta_sea_m = 0 && tau_cmd_in > 0)
		 // sigma_adap_fric = 1;
		 else if (dt_theta_sea_m < 0 )// || (dt_theta_sea_m = 0 && tau_cmd_in < 0))
		 {
		    sigma_adap_fric = -1;
			}
		 // else if (dt_theta_sea_m = 0 && tau_cmd_in < 0)
		...
		err_theta  = deflection_sea_ref  - theta_sea; 
		err_torque = torque_ref - torque_sea; 
		err_q      = q_ref - theta_sea_out; 
	  ...
	  tf_discr_io_help(&tau_cas_pi_theta, B_cas_torq, A_cas_torq, err_theta, error_theta_arr, tau_cas_pi_theta_arr, ORD_Cas_PI); // theta_sea as outer-loop
      //tf_discr_io_help(&tau_cas_pi_torq, B_cas_torq, A_cas_torq, err_torque, error_torq_arr, tau_cas_pi_torq_arr, ORD_Cas_PI);  // torque_sea as outer-loop
	  ...
	  tau_cas_pi_in = tau_cas_pi_theta - tau_cas_pi_vel; 
     ...
     // Feedback linearization-based SMC 
        dh_dtheta_ord1 = diff_h_theta_ord1(PRE_EXTENSION_LEGENTH, OFFSET_ANGLE, theta_sea);
	    dh_dtheta_ord2 = diff_h_theta_ord2(PRE_EXTENSION_LEGENTH, OFFSET_ANGLE, theta_sea);

		alpha_FL = (double) dh_dtheta_ord2*pow(dt_theta_sea,2);
     ...
     // NDOBC torque controller
     // tf_discr_io_ord1(&dt_theta_sea_m, B_TDEMFC_diff, A_TDEMFC_diff, theta_sea_m, &theta_sea_m_prev, &dt_theta_sea_m_prev); 
     pz_NDOB      = gama_NDOB*J_sea_NDOB*dt_theta_sea;
	...
	// Cascade PI position controller
     tf_discr_io_ord1(&Desire_cas_pi_outer_POS, B_cas_outer_POS, A_cas_outer_POS, err_q, &error_cas_outer_POS_prev, &Desire_cas_pi_outer_POS_prev); 
	// error_cas_inner_POS = Desire_cas_pi_outer_POS - theta_sea_m;   // position source 
   	 error_cas_inner_POS = Desire_cas_pi_outer_POS - dt_theta_sea_m;  // velocity-source
	 tf_discr_io_ord1(&tau_cas_pi_in_POS, B_cas_inner_POS, A_cas_inner_POS, error_cas_inner_POS, &error_cas_inner_POS_prev, &tau_cas_pi_in_POS_prev); 
    // tau_cas_pi_in_damping_POS = tau_cas_pi_in_POS - 0.01*dt_torque_sea; 
     tau_cas_pi_in_damping_POS = tau_cas_pi_in_POS - 0.01*dt_theta_sea;
    
    // Direct PID position controller
	 tf_discr_io_help(&tau_dir_PID_POS, B_dir_PID_POS, A_dir_PID_POS, err_q, error_q_dir_PID_POS_arr, tau_dir_PID_POS_arr, ORD_dir_PID_POS);
	 tau_dir_PID_SingPertur_POS = tau_dir_PID_POS - 0.0005*dt_theta_sea; 

    // NDOBC for position control
    // pz_NDOB      = gama_NDOB*J_sea_NDOB*dt_theta_sea;
    // z_NDOB_lump  = torque_sea - u_NDOBC_in - pz_NDOB;
	...
	////////////////////////////////////////////
     tau_cmd_in   = frac_dir_PID*tau_PID_torq - frac_dist*tau_dist_in + frac_cas_pi*tau_cas_pi_in 
				  ...
				  + frac_pos_Cascade_PI*tau_cas_pi_in_damping_POS + frac_pos_Direct_PID*tau_dir_PID_SingPertur_POS;
	...
	// curr_ref = CURR_AMP_REF*(1.0 - exp( -N_T*(t_s - t_step_s)/T_EXP_R/2.0) );
		...
		if ((cycle_count % N_CYCLES_DISP) == 0) {
			...
			printf("theta_sea   = [%3.4lf]  theta_sea_in = [%3.4lf]  theta_sea_out = [%3.4lf] q_ref = [%3.4lf] \n", theta_sea, theta_sea_in, theta_sea_out, q_ref); 
			printf("torque_ref = [%3.4lf]  torque_sea  = [%3.4lf]  dt_theta_sea_m  = [%3.4lf]  \n", torque_ref, torque_sea, dt_theta_sea_m);
